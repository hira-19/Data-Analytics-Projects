...
SELECT 
    major,
    COUNT(CASE WHEN gpa >= 3.5 THEN 1
    END) AS high,
COUNT(CASE WHEN gpa between 2.5 and 3.49 THEN 1
    END) AS Medium,
    COUNT(CASE WHEN gpa < 2.5 THEN 1
    END) AS Low from university_students_dataset group by major;
